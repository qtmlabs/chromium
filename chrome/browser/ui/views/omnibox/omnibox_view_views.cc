...
bool OmniboxViewViews::OnBeforeCutOrCopy(views::Textfield* sender,
  ...
  GURL url;
  bool write_url;
  *copy_contents = GetSelectedText();
  controller()->edit_model()->AdjustTextForCopy(
      GetSelectedRange().GetMin(), copy_contents, &url, &write_url);
  return true;
}
...
OmniboxViewViews::CreateClipboardWriter() {
  auto scoped_clipboard_writer = std::make_unique<ui::ScopedClipboardWriter>(
      ui::ClipboardBuffer::kCopyPaste);
  if (!ShouldDoLearning()) {
    scoped_clipboard_writer->MarkAsOffTheRecord();
  }
  return scoped_clipboard_writer;
}
